...
public:
    std::size_t operator()(const CTxDestination& t) const {
	  boost::hash<std::string> hasher;
	  std::string ts = CBitcoinAddress(t).ToString();
          return hasher(ts);
    }

    bool operator()(const CTxDestination& a, const CTxDestination& b) const {
	return a == b;
    }
};
...
struct CKeyIDHasher {
public:
    std::size_t operator()(const CKeyID& t) const {
	  boost::hash<std::string> hasher;
	  std::string ts = t.ToString();
          return hasher(ts);
    }

    bool operator()(const CKeyID& a, const CKeyID& b) const {
	return a == b;
    }
};
