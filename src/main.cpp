...
unsigned int nTargetSpacing     = 61;               // 61 seconds
...
const string strMessageMagic = "Quotient Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    if (nHeight == 1)
        nSubsidy = 16180 * COIN; // 1%
    else
    if (nHeight <= LAST_FAIR_LAUNCH_BLOCK) 
        nSubsidy = 1 * COIN;
    else
    if (nHeight <= LAST_POW_BLOCK && nHeight <= 500)
        nSubsidy = 618 * COIN;
    else
    if (nHeight <= LAST_POW_BLOCK && nHeight <= 1000 && nHeight > 500)
        nSubsidy = 309 * COIN;
    else
    if (nHeight <= LAST_POW_BLOCK && nHeight <= 6000 && nHeight > 1000)
        nSubsidy = 154 * COIN;
    else
    if (nHeight <= LAST_POW_BLOCK && nHeight <= 6500 && nHeight > 6000)
        nSubsidy = 309 * COIN;
    else
    if (nHeight <= LAST_POW_BLOCK && nHeight > 6500)
        nSubsidy = 618 * COIN;
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Quotient", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block        
        const char* pszTimestamp = "11/10/2014 CNBC.com - Alibaba: $1B in Singles Day sales in 17 minutes";
        CTransaction txNew;
        txNew.nTime = 1415652402;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1415652402;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 3158 : 3158;
	...
	printf("block.nNonce = %u \n", block.nNonce);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x83031c1f8f8be9f80bcc6f3795338a0282a2bac520af3889636ddbc26aa28805"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xef, 0xb2, 0xfa, 0xf2 };
